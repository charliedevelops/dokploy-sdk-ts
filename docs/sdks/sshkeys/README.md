...
async function run() {
  const result = await dokploy.sshKeys.generate({});
...
import { sshKeysGenerate } from "dokploy-sdk/funcs/sshKeysGenerate.js";
...
async function run() {
  const res = await sshKeysGenerate(dokploy, {});
  if (res.ok) {
    const { value: result } = res;
    console.log(result);
  } else {
    console.log("sshKeysGenerate failed:", res.error);
  }
}
