...
async function run() {
  const result = await dokploy.bitbucket.get.providers();
...
import { bitbucketGetProviders } from "dokploy-sdk/funcs/bitbucketGetProviders.js";
...
async function run() {
  const res = await bitbucketGetProviders(dokploy);
  if (res.ok) {
    const { value: result } = res;
    console.log(result);
  } else {
    console.log("bitbucketGetProviders failed:", res.error);
  }
}
