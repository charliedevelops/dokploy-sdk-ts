...
import { DokployDefaultError } from "../models/errors/dokploydefaulterror.js";
...
export function match<T, E>(
...
): MatchFunc<T, E | DokployDefaultError | ResponseValidationError> {
  return async function matchFunc(
    ...
    [
      result: Result<T, E | DokployDefaultError | ResponseValidationError>,
      raw: unknown,
    ]
  > {
    let raw: unknown;
    ...
    if (!matcher) {
      return [{
        ok: false,
        error: new DokployDefaultError("Unexpected Status or Content-Type", {
          response,
          request,
    ...
    if (matcher.enc === "fail") {
      return [{
        ok: false,
        error: new DokployDefaultError("API error occurred", {
          request,
          response,
          body,
        }),
